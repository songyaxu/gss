			...
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						...
						<li>
						<a href='#subPages3' data-toggle='collapse' class='active'><i class='lnr lnr-file-empty'></i> <span>管理员管理</span> <i class='icon-submenu lnr lnr-chevron-left'></i></a>
							<div id='subPages3' class='collapse in'>
								<ul class='nav'>
									<c:if test="${user.id=='admin'}">
										<li><a href='${pageContext.request.contextPath}/manage/admin/add' class=''>新增</a></li>
									</c:if>
									<li><a href='${pageContext.request.contextPath}/admin/scan' class='active'>编辑/查看</a></li>
								</ul>
							</div>
						</li>
					</ul>
					...
					<div id="toastr-demo" class="panel">
						<div class="panel-body">
							<table class="table table-bordered">
								...
								<tbody>
									<c:forEach var="varName" items="${adminList}" varStatus="status">
						...
						<div class="panel-footer pagenav">		
							<ul>
								<c:if test="${adminPage.hasPrePage==true}">
								<li><a href="${pageContext.request.contextPath}/admin/scan/page/${adminPage.currentPage-1}" class="btu">上一页</a></li>
								</c:if>
								<li><a href="#" class="btu active">${adminPage.currentPage}</a></li>
								<c:if test="${adminPage.currentPage+1<=adminPage.totalPage}">
									<li><a href="${pageContext.request.contextPath}/admin/scan/page/${adminPage.currentPage+1}" class="btu">${adminPage.currentPage+1}</a></li>
								</c:if>
								<c:if test="${adminPage.currentPage+2<=adminPage.totalPage}">
									<li><a href="${pageContext.request.contextPath}/admin/scan/page/${adminPage.currentPage+2}" class="btu">${adminPage.currentPage+2}</a></li>
								</c:if>
								<c:if test="${adminPage.currentPage+3<=adminPage.totalPage}">
									<li><a href="${pageContext.request.contextPath}/admin/scan/page/${adminPage.currentPage+3}" class="btu">${adminPage.currentPage+3}</a></li>
								</c:if>
								<c:if test="${adminPage.hasNextPage==true}">
									<li><a href="${pageContext.request.contextPath}/admin/scan/page/${adminPage.currentPage+1}" class="btu" >下一页</a></li>
									<li><a href="${pageContext.request.contextPath}/admin/scan/page/${adminPage.totalPage}" class="btu">最后一页</a></li>
								</c:if>
							</ul>
						</div>
					</div>
	    ...
	    function reset(id){
	    	layer.confirm('确定要重置用户'+id+'的密码吗？', {
	    		  btn: ['确定','取消']
	    			...
	    			$.get("${pageContext.request.contextPath}/user/resetPwd",{ id: id,type:3},
	    	    		    function(data){
	    		  ...
	    		  btn: ['确定','取消']
	    			...
	    			$.get("${pageContext.request.contextPath}/user/delete/"+id,
	    	        		...
	    	        		if(data.statusCode=="1"){
	    	        			layer.msg(data.message, {icon: 1});
	    	        			setTimeout(function () {
	    	                        window.location = "${pageContext.request.contextPath}/admin/scan";
	    	                    }, 1500);
	    	        		}
	    	        		else{
	    	        	        layer.msg(data.message, {icon: 5});

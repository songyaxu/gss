...
<body>
					...
					<ul class="nav">
						...
						<a href='#subPages2' data-toggle='collapse' class='active'><i class='lnr lnr-file-empty'></i> <span>教师管理</span> <i class='icon-submenu lnr lnr-chevron-left'></i></a>
							<div id='subPages2' class='collapse in'>
								<ul class='nav'>
									<li><a href='${pageContext.request.contextPath}/manage/teacher/add' class='active'>新增</a></li>
    ...
    function add(){
        ...
        if(id==null||id=="")
        {
        	layer.msg("教师ID不能为空", {icon: 5});
        	return;
        }
        if(name==null||name=="")
        {
        	layer.msg("教师姓名不能为空", {icon: 5});
        	return;
        }	
        $.get("${pageContext.request.contextPath}/user/checkExist",{ id: id},
        		...
        		if(data.statusCode=="1"){
        	    	...
        	    	$.get("${pageContext.request.contextPath}/teacher/add",{ id: id, name: name, password: password },
        	    		    function(data){
        	        		if(data.statusCode=="1"){

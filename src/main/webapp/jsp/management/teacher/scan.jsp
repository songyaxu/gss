			...
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						...
						<li>
						<a href='#subPages2' data-toggle='collapse' class='active'><i class='lnr lnr-file-empty'></i> <span>教师管理</span> <i class='icon-submenu lnr lnr-chevron-left'></i></a>
							<div id='subPages2' class='collapse in'>
								<ul class='nav'>
									<li><a href='${pageContext.request.contextPath}/manage/teacher/add' class=''>新增</a></li>
									<li><a href='${pageContext.request.contextPath}/teacher/scan' class='active'>编辑/查看</a></li>
								</ul>
							</div>
						</li>
						<li>
					...
					<div id="toastr-demo" class="panel">
						<div class="panel-body">
							<table class="table table-bordered">
								...
								<tbody>
									<c:forEach var="varName" items="${teaList}" varStatus="status">
										<tr <c:if test="${status.index%2==0}">style="background-color: snow"</c:if>>
							                ...
							                <td>${varName.phone}</td>
							                <td>
							                	<c:if test="${varName.type==0}">教师</c:if>
							                	<c:if test="${varName.type==1}">答辩秘书</c:if>
											</td>
							                <td>${(varName.lastLoginTime eq null) ? "从未登陆":varName.lastLoginTime}</td>
							                <td>
							                	<a href="${pageContext.request.contextPath}/manage/teacher/${varName.id}">编辑</a>
							                	<a href="javascript:void(0);" onclick="reset('${varName.id}')">重置密码</a>
						...
						<div class="panel-footer pagenav">		
							<ul>
								<c:if test="${teaPage.hasPrePage==true}">
								<li><a href="${pageContext.request.contextPath}/teacher/scan/page/${teaPage.currentPage-1}" class="btu">上一页</a></li>
								</c:if>
								<li><a href="#" class="btu active">${teaPage.currentPage}</a></li>
								<c:if test="${teaPage.currentPage+1<=teaPage.totalPage}">
									<li><a href="${pageContext.request.contextPath}/teacher/scan/page/${teaPage.currentPage+1}" class="btu">${teaPage.currentPage+1}</a></li>
								</c:if>
								<c:if test="${teaPage.currentPage+2<=teaPage.totalPage}">
									<li><a href="${pageContext.request.contextPath}/teacher/scan/page/${teaPage.currentPage+2}" class="btu">${teaPage.currentPage+2}</a></li>
								</c:if>
								<c:if test="${teaPage.currentPage+3<=teaPage.totalPage}">
									<li><a href="${pageContext.request.contextPath}/teacher/scan/page/${teaPage.currentPage+3}" class="btu">${teaPage.currentPage+3}</a></li>
								</c:if>
								<c:if test="${teaPage.hasNextPage==true}">
									<li><a href="${pageContext.request.contextPath}/teacher/scan/page/${teaPage.currentPage+1}" class="btu" >下一页</a></li>
									<li><a href="${pageContext.request.contextPath}/teacher/scan/page/${teaPage.totalPage}" class="btu">最后一页</a></li>
								</c:if>
							</ul>
						</div>
					</div>
	    ...
	    function reset(id){
	    	layer.confirm('确定要重置用户'+id+'的密码吗？', {
	    		  btn: ['确定','取消']
	    			...
	    			$.get("${pageContext.request.contextPath}/user/resetPwd",{ id: id,type:1},
	    	    		    function(data){
	    		  ...
	    		  btn: ['确定','取消']
	    			...
	    			$.get("${pageContext.request.contextPath}/user/delete/"+id,
	    	        		...
	    	        		if(data.statusCode=="1"){
	    	        			layer.msg(data.message, {icon: 1});
	    	        			setTimeout(function () {
	    	                        window.location = "${pageContext.request.contextPath}/teacher/scan";
	    	                    }, 1500);
	    	        		}
	    	        		else{
	    	        	        layer.msg(data.message, {icon: 5});

...
<body>
							...
							<div class="profile-left">
									...
									<div class="profile-stat">
										<div class="row">
											...
											<div class="col-md-4 stat-item">
												...
												<c:if test="${topic.secretaryName==''}"><a href="javascript:setSecretary()" color="red;">点击设置</a></c:if>
												</span>
											</div>
											<div class="col-md-4 stat-item">
								...
								<div class="profile-detail">
									...
									<c:if test="${topic.attachment!=0 }">
										...
										<div class="text-center"><a href="javascript:check(${topic.attachment})" class="btn btn-success"><i class="fa fa-download"></i>下载附件</a></div>
									</c:if>
								</div>
								<!-- END PROFILE DETAIL -->
							</div>
								...
								<div class="tab-content">
									...
									<div class="tab-pane fade" id="tab-bottom-left4">
										<ul class="list-unstyled activity-timeline">
											...
											<c:if test="${topic.status >= 5}">
												...
												<c:forEach var="msg" items="${msgList4 }" varStatus="status">
											<li>
												<c:if test="${msg.type==1 }">
													<i class="fa fa-line-chart activity-icon"></i>
													<p>${msg.sendId }${msg.title }: ${msg.content }  <span class="timestamp">${msg.time }</span></p>
									...
									<div class="tab-pane fade" id="tab-bottom-left5">
										<ul class="list-unstyled activity-timeline">
											...
											<c:if test="${topic.status >= 6}">
												...
												<c:forEach var="msg" items="${msgList5 }" varStatus="status">
											<li>
												<c:if test="${msg.type==1 }">
													<i class="fa fa-line-chart activity-icon"></i>
													<p>${msg.sendId }${msg.title }: ${msg.content }  <span class="timestamp">${msg.time }</span></p>
									...
									<div class="tab-pane fade" id="tab-bottom-left6">
										<ul class="list-unstyled activity-timeline">
											...
											<c:if test="${topic.status >= 7}">
												<c:forEach var="msg" items="${msgList6 }" varStatus="status">
											<li>
												<c:if test="${msg.type==1 }">
													<i class="fa fa-line-chart activity-icon"></i>
													<p>${msg.sendId }${msg.title }: ${msg.content } <span class="timestamp">${msg.time }</span></p>
									...
									<div style="text-align: center; padding-bottom: 20px;">
										<button type="button" class="btn btn-primary" onclick="endstage();"><i class="fa fa-check-circle"></i> 结束当前流程</button>
									</div>
	...
	<script type="text/javascript">
		...
		function setSecretary(){
			layer.prompt({title: '请输入答辩秘书id', formType: 3}, function(pass, index){
				  layer.close(index);
				  $.get("${pageContext.request.contextPath}/topic/setsecretary/"+${topic.id},{tid:pass},
			    		    function(data){
			        		if(data.statusCode=="1"){
			        			layer.msg(data.message, {icon: 1});
			        			$("#secretary").html(data.data);
			        		}
			        		else{
			        	        layer.msg(data.message, {icon: 5});
			        		}
		...
		function send(){
			...
			$.get("${pageContext.request.contextPath}/message/send",
					{title:"评论了",type:2,content:content,receiveId:${topic.studentId},topicId:${topic.id}},
	    		    function(data){
	        		if(data.statusCode=="1"){
	        			layer.msg(data.message, {icon: 1});
	        			$("#content").val("");
		...
		function endstage(){
			layer.confirm('你确定要结束当前流程吗？', {
				  btn: ['确定','取消'] //按钮
				}, function(){
					$.get("${pageContext.request.contextPath}/topic/endstage/"+${topic.id},
			    		    function(data){
			        		if(data.statusCode=="1"){
			        			layer.msg(data.message, {icon: 1});
			        			setTimeout(function () {

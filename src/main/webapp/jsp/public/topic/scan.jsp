										...
										<tr <c:if test="${status.index%2==0}">style="background-color: snow"</c:if>>
							                ...
							                <c:if test="${user.type==0}">
							                	<td>
							                		<c:if test="${varName.status==0}">
							                			<c:if test="${stuStatus==0}"><a href="javascript:choice(${varName.id})">选择</a></c:if>
							                			<c:if test="${stuStatus!=0}">不可选</c:if>
							                		</c:if>
							                		<c:if test="${varName.status!=0}">不可选</c:if>
							                ...
							                <c:if test="${user.type!=0}">
							                	<td>
							                		...
							                		<c:if test="${varName.status==1}">不可选</c:if>
   	        		...
   	        		if(data.statusCode=="1"){
   	        			$.get("${pageContext.request.contextPath}/topic/choice/"+id,
        	        		...
        	        		if(data.statusCode=="1"){
        	        			...
        	        			setTimeout(function () {
        	                        window.location = "${pageContext.request.contextPath}/topic/scan/page/${topPage.currentPage-1}";
        	                    }, 1500);
        	        		}
        	        		else{
        	        	        layer.msg(data.message, {icon: 5});
